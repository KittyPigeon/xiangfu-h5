<template>
  <div class="home-page">
    ...
    <div class="home-content" :class="{ expanded: isExpanded }">
      ...
      <ActivityTab
        v-if="isExpanded"
        class="activity-tab-top"
        @change="handleActivityTabChange"
      />
      ...
      >
        ...
        >
          <!-- 活动 Banner 组件 -->
          <ActivityBanner
            :banner-image="item.coverImageSrc"
...
const handleFavorite = async flag => {
  ...
  const [err, res] = flag
    ? await to(addCollect(params))
    : await to(delColloect(params));
  if (err) {
...
const getActivityList = async () => {
  const params = {
    queryDTO: {
      ...
      categoryId:
        tabIndex.value === -1 ? null : tabList.value[tabIndex.value].id,
  ...
  activityList.value = res.data.records.map((item, index) => {
    ...
    // const baseLatitude = 30.306876;
    // const baseLongitude = 120.092841;
    // const offset = index * 0.01; // 为每个活动设置不同的偏移量

    return {
      ...item,
      coverImageSrc: !item.coverImage.includes("http")
...
const openActivityDetail = async data => {
  ...
  res.data.imagesSrcArr = (res.data.imageList ? res.data.imageList : []).map(
    o => {
  ...
  res.data.isFavorited =
    res2 && res2.data && res2.data.checkResult ? res2.data.checkResult : false;
