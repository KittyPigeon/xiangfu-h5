...
import {
  // queryHotMerchantCategory,
  queryMerchantHomeSearch
  // queryMerchatPage
} from "@/api/shop";
  ...
  AMapLoader.load({
    ...
    .then(async AMap => {
      ...
      map = new AMap.Map("map-container", {
        zoom: 11, // 级别
        center: JSON.parse(window.localStorage.getItem("mylocation") || "{}")
          .locatonArr, // 中心点坐标
...
const markerListFn = (options = { isGuide: false }) => {
  ...
  merchatList.value.forEach(item => {
    if (!markerMap.value.has(item.id)) {
      ...
      console.log("markerListFn - 创建新标记:", {
        id: item.id,
        position,
        title
      });
...
<template>
  ...
  <div class="home-content">
    ...
    <div class="activity-control" @click.stop>
      ...
      <!-- 收藏弹窗 -->
      <DragExpandPanel
        class="drag-panel-container"
        title="分类筛选&收藏筛选"
        @drag="handleDrag"
      >
        <StartList
          :is-expanded="isExpanded"
          :filter-items-list="categoryList"
