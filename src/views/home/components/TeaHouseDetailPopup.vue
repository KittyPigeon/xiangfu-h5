      ...
      <div class="info-section" @click.stop>
        ...
        <div class="meta-info">
          <div class="meta-info-item">
            <span class="label address-label">地址：</span>{{ props.address }}
          </div>
          <div class="meta-info-item">
            <span class="label">营业时间：</span>{{ props.businessHours }}
          </div>
          <div class="meta-info-item rating">
            <span class="label">{{ props.commentCount }}条评论</span
            >{{ props.checkinCount }}次打卡
          </div>
        </div>
      </div>
        ...
        <div class="subsidy-card">
          <div class="subsidy-desc">
            ...
            <div class="subsidy-title">
              【吃喝玩乐】神券{{ couponData.name }}
            </div>
            <div class="subsidy-expire">
              有效期至：{{ couponData.subsidyExpire }}
            </div>
            <div
              v-if="!couponData.isReceived"
              class="claim-btn"
              @click.stop="handleClaim"
            >
              去领取
            </div>
            <div v-else class="claimed-btn">已领取</div>
          </div>
          <div class="subsidy-amount">
            <span class="unit">¥</span>
            <span class="amount"> {{ couponData.subsidyAmount }} </span>
          </div>
        </div>
      </div>
...
const getMerchantCouponDataFn = async () => {
  const [err, res] = await to<any, any>(
    getMerchantCouponData(props.merchantId)
  );
  if (err) {
...
.title-row {
  .active-favoriteIcon {
    ...
    width: 20px;
    height: 20px;
    background: url("../../../assets/icons/icon-love-active.svg") no-repeat
      center;
  }
